package ru.sbrf.sbererp.core.common.reactive.unified.audit.metamodel;
...
import ru.sbrf.sbererp.core.common.reactive.unified.audit.converter.MetaModelConverter;
 ...
 *
 * <p>Ошибка регистрации логируется и не валит старт приложения.
 */
@Slf4j
@Component
public final class MetaModelSender {
  ...
  /**
   * Создаёт метамодель и регистрирует её в системе аудита.
   */
  @Order(AuditNumericConstants.METAMODEL_REGISTER_ORDER)
  @EventListener(ApplicationReadyEvent.class)
  public void register() {
    final Metamodel metamodel = metaModelConverter.create();
    final List<EventMetaInfo> eventMetaInfos = Option.<List<EventMetaInfo>>of(metamodel.getEventMetaInfos())
        .getOrElse(List.of());
    log.info(
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          AuditLogMessages.METAMODEL_EVENT_NAMES,
          eventMetaInfos.stream()
              .map(EventMetaInfo::getName)
              .collect(Collectors.joining(AuditTextConstants.COMMA))
      );
      log.debug(
          AuditLogMessages.AUDIT_METAMODEL_PAYLOAD,
          AuditPrettyJsonUtils.getFormatString(metamodel, AuditLogMessages.FAILED_TO_SERIALIZE_METAMODEL)
      );
    }
